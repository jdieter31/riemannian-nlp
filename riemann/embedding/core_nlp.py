...
class SimpleToken:
    ...
    def to_proto(self) -> CoreNLP_pb2.Document:
        proto = CoreNLP_pb2.Token()
        self.fill_proto(proto)
        return proto

    @classmethod
    def detokenize(cls, tokens: Sequence['SimpleToken']) -> str:
        """
...
class SimpleSentence:
    ...
    def to_proto(self) -> CoreNLP_pb2.Sentence:
    ...
    def from_proto(cls, proto: CoreNLP_pb2.Sentence) -> 'SimpleSentence':
        ...
        if proto.text:
            ...
            # Regenerate text if proto doesn't contain it.
            text = "".join(token.originalText + token.after for token in proto.token)
        return cls(
            text=text,
            tokens=tokens,
    ...
    def from_text_tokens(
        ...
        :return: A SimpleSentence
        """
        SPECIAL_TOKENS: List[Tuple[str, List[str]]] = [
            ("-LRB-", ["("]),
            ("-RRB-", [")"]),
            ("-LSB-", ["["]),
            ("-RSB-", ["]"]),
