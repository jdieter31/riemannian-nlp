...
class ConfigDictTest(unittest.TestCase):
    ...
    def test_parsing(self):
        ...
        # String quoting
        self.assertEqual({'name': 'the "best" name'}, ConfigDictParser.parse('name="the \\"best\\" name"'))
        self.assertEqual({'classes': ["a.b", "b.c", "c.d", "none"]}, ConfigDictParser.parse('classes = [a.b, b.c, c.d, "none"]'))
        ...
        # Lists
        self.assertEqual({'ff_dim': [128, 128, 512]}, ConfigDictParser.parse('ff_dim=[128, 128, 512]'))
        self.assertEqual({'classes': ["general.yes", "general.no"]}, ConfigDictParser
                         .parse('classes=[\"general.yes\",\"general.no\"]'))
 
        # Hierarchical 1
        self.assertEqual({'optimizer': {"type": "adam"}}, ConfigDictParser.parse('optimizer.type=adam'))
 
        # Hierarchical 2
        self.assertEqual({'optimizer': {
            "type": "adam", "lr": 1e-4, "betas": [1e-4, 1e-5], "scheduler": {"type": "ReduceLROnPlateau"}}
        }, ConfigDictParser.parse('optimizer=(type=adam lr=1e-4 betas=[1e-4, 1e-5] scheduler=(type=ReduceLROnPlateau))')
        )
