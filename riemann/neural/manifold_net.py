...
class ManifoldNetwork(nn.Module):
    def __init__(
        ...
        for i in range(len(manifold_seq) - 1):
            if i == len(manifold_seq) - 2:
                layer_list.append(
                    ManifoldLayer(manifold_seq[i], manifold_seq[i + 1], dimension_seq[i],
                                  dimension_seq[i + 1], None, num_poles, log_base_inits[i],
                                  exp_base_inits[i]))
            else:
                layer_list.append(
                    ManifoldLayer(manifold_seq[i], manifold_seq[i + 1], dimension_seq[i],
                                  dimension_seq[i + 1], non_linearity, num_poles, log_base_inits[i],
                                  exp_base_inits[i]))
    ...
    def from_save_data(cls, data):
        params = data['params']
        params += [[None for _ in range(len(params[0]) - 1)],
                   [None for _ in range(len(params[0]) - 1)]]
